...
const templateChangeReminder = (id,reminder) => {
    return `UPDATE userevents SET status = '${reminder}' WHERE userevents.id = ${id}`
}

function doneEvent(param, send) {
    poll.execute(templateChangeReminder(param.idEvent, param.status ? 1 : 0))
        .then(res=> {
            if(res[0]) {
